    ...
    if request.method =='POST':  
        if get_role=="User" and perm.interest_add ==True or get_role=="Admin" or rejin.is_superuser == True:    
            ...
            if serializer876.is_valid():
                ...
                #
                # NOTE (Sep 2026): this block is UNGUARDED against the
                # legacy backdated catch-up block further down, which also
                # adds to intrest_amt when the loan is backdated more than a
                # month AND apply_first_interest is False. That combination
                # double-counts for Chit fund Interest — a pre-existing
                # issue, left as-is here since this change is scoped to
                # Management Interest only (see guarded block below for the
                # fixed pattern).
                # ------------------------------------------------------------
                ...
                # ------------------------------------------------------------
                # Owner rule (Sep 2026): populate
                #   balancesheet_peopleinterestbalancesheet.intrest_amt
                # from the Fix Interest Rate for Management Interest loans
                # too (categories "Interest" and "Interest with capital"),
                # mirroring the Chit fund Interest fix directly above.
                # Previously, for Management Interest, intrest_amt was only
                # ever written when apply_first_interest = True (copied
                # straight from temp_family.interest_amt) — a freshly
                # created Management Interest loan that does NOT apply its
                # first period's interest immediately was left showing 0
                # interest on its balance sheet until the legacy overdue
                # catch-up block ran a month later.
                #
                # Rule (from owner spec — same as the Chit fund case):
                #   Fix Interest Rate "percentage" -> intrest_amt =
                #       (fix_interest_rate_percent × principal_amt) / 100
                #   Fix Interest Rate "amount"      -> intrest_amt =
                #       fix_interest_rate_percent   (flat ₹ value)
                #
                # Scoped strictly to interest_type == "Management Interest"
                # and interest_category in ("Interest", "Interest with
                # capital"). Installment Interest is excluded — its interest
                # is already baked into the fixed installment_amt per period
                # and must not additionally accrue this separate charge.
                #
                # DOUBLE-COUNT GUARD (Sep 2026 fix):
                # The legacy catch-up block further below (guarded by
                # `date_object.date() + relativedelta(months=1) <= today()
                # and not temp_family.apply_first_interest`) ALSO adds a
                # freshly computed period of interest onto
                # `inter_bal.intrest_amt` for Management Interest records
                # with interest_category != "Installment Interest". If this
                # block here had already set intrest_amt to one period's
                # worth, the catch-up block would stack a second period on
                # top of it — doubling intrest_amt / intrest_balance_amt /
                # credit_amt / balance_amt for that loan.
                #
                # `_mgmt_catchup_will_run` mirrors the exact condition the
                # catch-up block itself checks, so this block only writes
                # intrest_amt when the catch-up block will NOT also run for
                # this record — the two paths are now mutually exclusive.
                # ------------------------------------------------------------
                _mgmt_catchup_will_run = (
                    date_object.date() + relativedelta(months=1) <= datetime.date.today()
                    and not temp_family.apply_first_interest
                )
                if (temp_family.interest_type == "Management Interest"
                        and temp_family.interest_category in ("Interest", "Interest with capital")
                        and not _mgmt_catchup_will_run):
                    fix_rate_mgmt  = float(temp_family.fix_interest_rate_percent or 0)
                    principal_mgmt = float(bal_sheet.principal_amt or 0)
                    if (temp_family.interest_type_new or "").lower() == "percentage":
                        computed_interest_mgmt = round((fix_rate_mgmt * principal_mgmt) / 100.0, 2)
                    else:  # "amount"  ->  flat ₹ value
                        computed_interest_mgmt = round(fix_rate_mgmt, 2)
                    bal_sheet.intrest_amt = computed_interest_mgmt
                    if temp_family.apply_first_interest == True:
                        bal_sheet.debit_amt = computed_interest_mgmt
                        bal_sheet.intrest_paid_amt = computed_interest_mgmt
                    bal_sheet.save()
                ...
                # creating new ones.
                #
                # NOTE (Sep 2026): for Management Interest (Interest /
                # Interest with capital) records, this block's entry
                # condition is now exactly mirrored by `_mgmt_catchup_will_run`
                # above, which prevented the newer block from ALSO writing
                # intrest_amt for the same record — so when this block runs
                # for those records, intrest_amt is still 0 going in here,
                # and this remains the single source of truth for that first
                # period, same as before the new block was introduced.
                # ------------------------------------------------------------
