...
class PeopleInterestDetailsSerializer(serializers.ModelSerializer):
    ...
    def validate(self, validated_data):
        ...
        # ------------------------------------------------------------------
        # Owner rule (Sep 2026): the "Interest date cannot be greater than
        # today" block no longer applies to Management Interest records
        # whose category is "Interest with capital" or "Installment
        # Interest". These two combinations may now be submitted with a
        # future interest_date. Every other combination (plain Management
        # "Interest", and all Chit fund Interest categories) is unaffected
        # and still blocks future dates exactly as before.
        # ------------------------------------------------------------------
        interest_type = validated_data.get("interest_type")
        interest_category = validated_data.get("interest_category")
        skip_future_date_check = (
            interest_type == "Management Interest"
            and interest_category in ("Interest with capital", "Installment Interest")
        )

        if (
            not skip_future_date_check
            and validated_data.get('interest_date')
            and validated_data['interest_date'] > timezone.now().date()
        ):
