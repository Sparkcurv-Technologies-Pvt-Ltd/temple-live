            ...
            if serializer876.is_valid():
                ...
                for mem_tax in get_tax_members:
                    ...
                    if mem_report:
                        ...
                        tem_report=TempleMemberReport.objects.create(management_profile=management,members=mem_tax,death_tariff=temp_family,reportdate=timezone.localdate(),credit_amt=temp_family.death_tariff_amt,balance_amt=bal,type_choice="Death Tariff",created_by=rejin.id)
                    else:
                        tem_report=TempleMemberReport.objects.create(management_profile=management,members=mem_tax,death_tariff=temp_family,reportdate=timezone.localdate(),credit_amt=temp_family.death_tariff_amt,balance_amt=temp_family.death_tariff_amt,type_choice="Death Tariff",created_by=rejin.id)
...
def edit_death_details(request,pk):
    ...
    if request.method == 'GET':        
        ...
        if get_role=="Admin" or rejin.is_superuser == True or get_role=="User" and perm.death_edit==True: 
            ...
            if customer.penalty_apply_date!=None and customer.penalty_apply_date<= timezone.localdate():
                rj_dic={}
                try:
                    rj_dic['documents']=request.data['documents']
                except Exception:
    ...
    #     return Response({'message':"un-authenticate"},status.HTTP_401_UNAUTHORIZED)
        ...
        if get_role=="Admin" or rejin.is_superuser == True or get_role=="User" and perm.death_edit==True or get_role=="User" and perm.death_delete==True:
            ...
            if customer.penalty_apply_date<=timezone.localdate():
