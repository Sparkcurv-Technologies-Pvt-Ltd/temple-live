                ...
                if fund_details.fund_type=="Fund 21":                    
                    ...
                    for mem in fund_mem_pro:                        
                        ...
                        FundMemberReport.objects.create(management_profile=temp_family.management_profile,balancesheet=set_fund,fund=temp_family,fund_m=mem,
                                          reportdate=timezone.localdate(),credit_amt= amount_per_head,debit_amt=amount_per_head,type_choice="Fund Initial",created_by=rejin.id)                       
                        CollectionDetails.objects.create(fund_name=fund_details.fund_type,transaction_date=timezone.localdate(),member_name=mem.member_name,mobile_number=mem.mobile_no,created_by=rejin.id,bill_by_name=rejin.username,fund_type="Initial",pay_date=timezone.localdate(),transaction_type="Cash",amount=temp_family.per_head_collection_amount,management_profile=temp_family.management_profile,collaction_no=coll_no(),collection_category="Fund",funds=temp_family,fund_member=mem)                      
                        ...
                        for mem in fund_mem_pro: 
                            ...
                            if member_check:
                                ...
                                if collection_check:
                                    ...
                                    coll_mem.pay_date=timezone.localdate()
                                    coll_mem.transaction_date=timezone.localdate()
...
def fund_lease_details(request):
    ...
    if request.method =='POST':
        ...
        if  get_role=="Admin" or rejin.is_superuser == True  or (get_role=="User" and perm.fund_add ==True):
            ...
            print(f_group)              
            today = timezone.localdate(); this_month = today.month
            print(this_month)
                        ...
                        if balance_check:
                            ...
                            if mem_report_check:
                                ...
                                FundMemberReport.objects.create(type_choice="Fund Lease",created_by=rejin.id,balancesheet=balance_last,fund_m=mem,management_profile=temp_family.management_profile,
                                                    reportdate=timezone.localdate(),fund=fund_check,credit_amt=temp_family.per_head_collection_amount,balance_amt=calculating_last_balance)
                    fund_mem_pro=FundLeaseMemberDetailss.objects.filter(flease=temp_family)
                    ...
                    for mem in take_mem:
                        ...
                        if balance_check:
                            ...
                            if mem_report_check:
                                ...
                                FundMemberReport.objects.create(type_choice="Fund Lease",created_by=rejin.id,balancesheet=balance_last,fund_m=mem,management_profile=temp_family.management_profile,
                                                    reportdate=timezone.localdate(),fund=fund_check,credit_amt=red_cal,balance_amt=calculating_last_balance)
                    fund_mem_pro=FundLeaseMemberDetailss.objects.filter(flease=temp_family)
                    for ssss in fund_mem_pro:
