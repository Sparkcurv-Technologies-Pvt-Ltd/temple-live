...
export const Interest = ({ RecordData, ClosUpdaForm, manageTrigger }) => {
    ...
    // Sep 2026 owner rule — percentage re-enabled alongside ₹ for Fix
    // Interest Rate (see SelectFixRate below), but ONLY for the
    // Management Interest + "Interest with capital" combination. Every
    // other Type/Category combo stays locked to ₹, same as before.
    // Default stays 'amount' so existing behaviour / legacy records are
    // unaffected unless the operator explicitly switches the selector to '%'.
    const [rateType, setRateType] = useState("amount");
    ...
    // Sep 2026 — resolves the "effective" interest type even before the
    // Type <CustomSelect> has fired onChange (ShowChooseType starts as
    // {} until touched, e.g. right after loading an existing record for
    // edit), falling back to whatever is already on RecordData.
    const effectiveInterestType =
        (typeof ShowChooseType === 'string' && ShowChooseType) || RecordData?.interest_type;

    // Sep 2026 owner rule — the %/₹ choice for Fix Interest Rate is only
    // meaningful for Management Interest + "Interest with capital".
    // Every other Type/Category combination (Management Interest +
    // Interest, Management Interest + Installment Interest, and all Chit
    // fund Interest categories) stays locked to ₹ only, same as the
    // pre-Sep-2026 behaviour.
    const isFixRateSelectable =
        effectiveInterestType === 'Management Interest' &&
        categoryChoose === 'Interest with capital';
    ...
    const handleCategory = (value) => {
        ...
        // Sep 2026 — if the new Type/Category combo no longer allows the
        // %/₹ choice, snap the Fix Interest Rate type back to ₹ so the
        // form/selector and downstream calculations stay consistent with
        ...
        if (!stillSelectable) {
            setRateType('amount');
            form.setFieldsValue({ interest_type_new: 'amount' });
        }
    }
    ...
    const handleChooseType = (value) => {
        ...
        // Sep 2026 — if the new Type/Category combo no longer allows the
        // %/₹ choice, snap the Fix Interest Rate type back to ₹.
        const stillSelectable =
            value === 'Management Interest' && categoryChoose === 'Interest with capital';
        if (!stillSelectable) {
            setRateType('amount');
            form.setFieldsValue({ interest_type_new: 'amount' });
        }
    ...
    useEffect(() => {
        if (RecordData) {
            ...
            // Sep 2026 — keep the Fix Interest Rate selector's local
            // `rateType` state in sync with the record's own
            // interest_type_new when opening an existing record for edit,
            // but only honour '%' when the record's own Type/Category is
            // actually the allowed combo (Management Interest + Interest
            // with capital). Otherwise force it back to 'amount' so a
            // stale/inconsistent record can't show the %/₹ selector where
            // it shouldn't appear.
            const recordAllowsPercentage =
                RecordData?.interest_type === 'Management Interest' &&
                RecordData?.interest_category === 'Interest with capital';

            if (RecordData?.interest_type_new && recordAllowsPercentage) {
                setRateType(RecordData?.interest_type_new)
            } else {
                setRateType('amount')
                form.setFieldsValue({ interest_type_new: 'amount' })
            }
    ...
    // Sep 2026 owner rule — percentage re-enabled for Fix Interest Rate,
    // alongside ₹, but ONLY for Management Interest + "Interest with
    // capital" (see `isFixRateSelectable`). `SelectFixRate` below picks
    // between this full `AmtType` list and the rupee-only list below
    ...
    // Sep 2026 — options list depends on whether the current Type/Category
    // combo allows choosing '%'; everything else stays locked to ₹.
    const SelectFixRate = (
        <CustomSelect options={isFixRateSelectable ? AmtType : FixRateOptionsRupeeOnly}
            initialValue={'amount'}
    ...
    return (
        <Form
            ...
            initialValues={{
                ...
                // Sep 2026 — Fix Interest Rate defaults to ₹ ('amount')
                // for new records, but the operator can now switch it to
                // '%' via SelectFixRate when the Type/Category combo
                // allows it (Management Interest + Interest with capital).
